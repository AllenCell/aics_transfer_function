...
class cyclelargeDataset(Dataset):
    ...
    def load_from_file(self, filenamesA, filenamesB, num_patch):
        ...
        if self.opt.network == 'stn' and self.opt.network["stn_adjust_fixed_z"]:
            ...
            assert os.path.isfile(self.opt.readoffsetfrom) , \
            ...
            with open(self.opt.readoffsetfrom, 'r') as fp:
                ...
                for key in fixed_dict1:
                    ...
                    clip_param = np.percentile(np.array(fixed_dict1[key])[:, 0],
                                               [10, 90]) 
                    offset_raw = []
                    ...
                    if z_std > 0.5:
                        print(f"WARNING: The standard deviation of offsets estimation\
                             for {key} is {z_std}. Not accurate!")
                        with open(self.opt.resultroot + 'WARNING', 'w') as wp:
                            wp.write(f"WARNING: The standard deviation of offsets \
    ...
    def apply_adjust(self, tensor, offsets_zyx):
        ...
        grid = torch.cat(
            [x_grid.unsqueeze(-1), y_grid.unsqueeze(-1), z_grid.unsqueeze(-1)],
            dim=-1
        ).unsqueeze(0)
