...
class ProjectTrainer(object):
    ...
    def __init__(self, opt):
        ...
        if opt.training_setting["imgs_per_epoch"] > \
    ...
    def run_trainer(self):
        ...
        total_epoch = self.opt.training_setting["niter"] + \
            self.opt.training_setting["niter_decay"] + 1
        for epoch in range(1, total_epoch):    
            ...
            idxA = random.sample(range(len(self.filenamesA)), 
                                 self.opt.training_setting["imgs_per_epoch"])
            ...
            dataset.load_from_file(
                fileA,
                fileB, 
                num_patch=self.opt.training_setting["patches_per_epoch"]
            )
                    ...
                    with open(self.opt.resultroot / Path('train.log'), 'a') as log_file:
                        ...
                        if self.opt.network["model"] in ['pix2pix']:
                            ...
                            if self.opt.save["save_training_inspections"]: 
                                save(self.opt.sample_dir / Path(
                                    str(total_iters) + 'rA.tiff'), rA)
                                save(self.opt.sample_dir / Path(
                                    str(total_iters) + 'rB.tiff'), rB)
                                save(self.opt.sample_dir / Path(
                                    str(total_iters) + 'fB.tiff'), fB)
                            ...
                            if self.opt.save["save_training_inspections"]: 
                                ...
                                save(self.opt.sample_dir / Path(
                                    str(total_iters) + 'rB.tiff'), rB)
                                save(self.opt.sample_dir / Path(
                                    str(total_iters) + 'fB0.tiff'), fB0)
                                save(self.opt.sample_dir / Path(
                                    str(total_iters) + 'fB.tiff'), fB)

                if total_iters % self.opt.save["save_latest_freq"] == 0 \
                   or total_iters == 100:
            ...
            tt = self.opt.training_setting['niter'] + \
